...
import conectaService from "./conectaService.js";
...
async function syncCapibas(userId, accessToken) {
  try {
    ...
    if (conectaUserData && typeof conectaUserData.balance === "number") {
      ...
      console.log(
        `Saldo de capibas do usuário ${userId} sincronizado para: ${conectaUserData.balance}`
      );
    } else {
      // Lança um erro se o saldo não for encontrado ou for inválido
      throw new Error(
        'O campo "balance" não foi encontrado ou é inválido nos dados da API externa.'
      );
    }
  } catch (error) {
    // Captura, loga e relança o erro para que a função chamadora possa tratá-lo
    console.error(
      `Erro ao sincronizar capibas para o usuário ${userId}:`,
      error.message
    );
